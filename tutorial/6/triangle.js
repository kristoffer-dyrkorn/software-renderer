...
export default class Triangle {
  ...
  draw(screenCoordinates, color) {
    ...
    let xmin = Math.trunc(Math.min(va[0], vb[0], vc[0])) - 1;
    let xmax = Math.trunc(Math.max(va[0], vb[0], vc[0])) + 1;
    let ymin = Math.trunc(Math.min(va[1], vb[1], vc[1])) - 1;
    let ymax = Math.trunc(Math.max(va[1], vb[1], vc[1])) + 1;
